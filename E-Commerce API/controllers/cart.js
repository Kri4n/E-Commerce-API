...
module.exports.addToCart = (req, res) => {
    Cart.findOne({ userId: req.user.id })
      .then(cart => {
        ...
        const existingItem = cart.cartItems.find(item => item.productId === req.body.productId);
        if (existingItem) {
  ...
  Cart.findOne({ userId: userId })
    .then(cart => {
      ...
      cart.save()
        ...
        .catch(err => res.status(500).send({ message: "Error saving cart.", error: err }));
    })
    .catch(err => res.status(500).send({ message: "Error finding cart.", error: err }));
};
